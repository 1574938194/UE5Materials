...
static constexpr auto _Meta_npos = ~size_t{ 0 };

template <class _List, class _Ty>
struct _Meta_find_index_ {
    using type = std::integral_constant<size_t, _Meta_npos>;
};
template <class _List, class _Ty>
...
struct _Meta_find_index_<_List<_First, _Rest...>, _Ty> {
    static constexpr bool _Bools[] = { std::is_same_v<_First, _Ty>, std::is_same_v<_Rest, _Ty>... };
    using type = std::integral_constant<size_t, _Meta_find_index_i_(_Bools, 1 + sizeof...(_Rest))>;
};
...
_NODISCARD constexpr bool holds_alternative(
    ...
    static_assert(_Idx != _Meta_npos, "holds_alternative<T>(const variant<Types...>&) requires T to occur exactly "
        "once in Types.");
    return _Var.get_type_index() == _Idx;
}
